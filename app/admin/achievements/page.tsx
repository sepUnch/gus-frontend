...
export default function ManageAchievementsPage() {
  ...
  const stats = {
    ...
    recent: achievements.length > 0 ? achievements[achievements.length - 1].name : "-"
  };
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Total Badges</p>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Badge Types</p>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{stats.types}</h3>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 flex items-center gap-4 shadow-sm">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-slate-500 dark:text-slate-400">Newest</p>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAchievements.map((ach) => {
                   ...
                   return (
                    <div 
                        key={ach.id} 
                        className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden cursor-pointer hover:-translate-y-1"
                    >
                        {/* Decorative Top Line */}
                        ...
                        <div className="flex items-start gap-4 mb-4 mt-2">
                            <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-3xl border border-slate-100 dark:border-slate-700 shadow-sm group-hover:scale-105 transition-transform">
                                {ach.icon_url || "🏆"}
                            </div>
                            <div className="flex-1 min-w-0 pt-1">
                                ...
                                {/* Type Badge */}
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${badgeColor}`}>
                                    {ach.type?.name === 'TRACK_COMPLETION' && <Target className="w-3 h-3" />}
                                    {ach.type?.name === 'SERIES_COMPLETION' && <Medal className="w-3 h-3" />}
                                    {!ach.type?.name?.includes('COMPLETION') && <Star className="w-3 h-3" />}
                        ...
                        {/* DESCRIPTION */}
                        <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 h-10 mb-4 leading-relaxed">
                            {ach.description}
                        </p>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center">
                ...
                <p className="text-slate-600 dark:text-slate-400 mb-6">
                  {searchTerm ? "Try adjusting your search query." : "Get started by creating your first achievement."}
                </p>
