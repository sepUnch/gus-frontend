...
export default function AdminSeriesDetailPage() {
  ...
  // Helper Variables
  const isExpired = series?.deadline ? new Date(series.deadline) < new Date() : false;
            ...
            {loading ? (
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-600 dark:text-slate-300 shadow-sm">
                                ...
                                <span>Track ID: <span className="font-semibold">{series.track_id}</span></span>
                            </div>
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-600 dark:text-slate-300 shadow-sm">
                                ...
                                <span>Deadline: <span className="font-semibold">{series.deadline ? new Date(series.deadline).toLocaleString() : "-"}</span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center items-center text-center">
                             ...
                             <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-2">Total Submissions</p>
                         ...
                         <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center items-center text-center overflow-hidden relative">
                             ...
                             {/* <div className="absolute top-0 w-full h-1 bg-gradient-to-r from-purple-400 to-pink-400"></div> */}
                             
                             <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-2">Verification Code</p>
                             {series.verification_code ? (
                                ...
                                <span className="text-sm font-semibold text-red-500 bg-red-50 px-3 py-1 rounded-full">Not Set</span>
                             )}
                        </div>
                    </div>
                </div>
                            ...
                            {submissions.map((sub: any) => {
                                ...
                                <tr key={sub.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                  ...
                                  <td className="px-6 py-4">
                                    ...
                                    >
                                      <svg className="w-4 h-4 flex-shrink-0 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                                      <span className="truncate">{sub.file_url ? "View Attachment" : "No Attachment"}</span>
                                    </a>
                                  </td>
               ...
               <div className="text-center py-20">
                   <h2 className="text-2xl font-bold text-slate-400">Series not found</h2>
               </div>
            )}
          </div>
        </main>
